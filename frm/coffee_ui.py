...
class CoffeeUI:
    ...
    def add_action(self,name,act,react_to=None):
        ...
        if isinstance(act,handleAction):
            self.actions[name]=act
        elif callable(act):
            self.actions[name] = handleAction(act,react_to=react_to)

    def add_subst(self,name,act):
        # a wrapper on top of handles.post
        # fills a lookup dict to substitute by user defied name
        if isinstance(act,handles.post):
            self.actions[name]=act
        else:
            self.actions[name] = handles.post(act)

    def handleAction(self,name,react_to=None):
        ...
        if r:
            if r.group(1)=='action':
                ...
                if t in self.states.keys():
                    ...
                    a = handleAction(self.states.get(t))
                else:
...
if __name__=='__main__':
    ...
    t.handleAction('get_buttons',handles.choose('role',
    coffee_encode(
    """
    student:
        'Show assignments':a 'ASSIGNMENTS'
        'Submit assignment':a 'SEND_ASSIGNMENT'
        'My progress':a 'PROGRES'
    applicant:
        Apply:a 'APPLY'""")
                          ))
    t.handleAction('react_to_lname',handleAction(lambda i,s,**d:\
                                    (States.LNAME,dict(d,fname=i.text)),
                                     react_to='text'))
    t.handleAction('name',handleAction(lambda i,s,**d:\
                                    (States.LNAME,dict(d,fname=i.text)),
                                     react_to='text'))
    ui = t.get_ui()
